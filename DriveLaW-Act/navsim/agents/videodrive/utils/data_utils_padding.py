...
#     mem_latents = vae.encode(padded_mem).latent_dist.sample(generator=generator)
...
def get_latents(
    ...
    """
    Encode history frames (mem) and future frames (video) with length alignment via temporal interpolation.
    
    Args:
        vae: Video VAE model
        mem: Tensor (B, C, m, H, W), m=4 history frames in [-1,1]
        video: Tensor (B, C, f, H, W), future frames in [-1,1]
        patch_size: spatial patch size for packing
        patch_size_t: temporal patch size for packing
        device: target device
        dtype: target dtype
        generator: randomness generator for sampling latents

    Returns:
        mem_latents: history latents of shape (B*m_packed) or (B, tokens, C)
        video_latents: future latents of shape (B, tokens, C)
    """

    device = device or vae.device
    B, C, m, H, W = mem.shape
    assert m == 4, f"Expected 4 history frames, got {m}"
    
    # ----------------------
    # 1) Future frames encode (保持原长度处理)
    # ----------------------
    video_latents = vae.encode(video).latent_dist.sample(generator=generator)
    video_latents = video_latents.to(dtype=dtype)
    video_latents = _normalize_latents(video_latents, vae.latents_mean, vae.latents_std)
    ...
    # ----------------------
    # 2) History frames padding: interpolate to 9 frames
    # ----------------------
    target_frames = 9
    # shape: (B, C, m, H, W) -> 插值时间维到 target_frames
    # trilinear 插值作用在 (T,H,W) 维
    mem_interp = F.interpolate(
        mem, size=(target_frames, H, W), mode='trilinear', align_corners=False
    )

    mem_latents = vae.encode(mem_interp).latent_dist.sample(generator=generator)
    mem_latents = mem_latents.to(dtype=dtype)
    mem_latents = _normalize_latents(mem_latents, vae.latents_mean, vae.latents_std)
    mem_latents = _pack_latents(mem_latents, patch_size, patch_size_t)
...
def gen_noise_from_condition_frame_latent(
    ...
    """
    以 token 级别构造 conditioning 噪声。
    输入 condition_frame_latent: (B, L_cond, C)
    返回 (latents, conditioning_mask, cond_indicator) 均为 token 级
    """
    # ---------- 准备工作 ----------
    B, L_cond, C = condition_frame_latent.shape          # token 级
    L_total = latent_num_frames * latent_height * latent_width

    # ---------- 1. 构造 token 级 init ----------
    init_latents = condition_frame_latent[:, :1, :].repeat(1, L_total, 1)   # (B, L_total, C)
    init_latents[:, :L_cond] = condition_frame_latent                       # 覆盖前 L_cond 个 token

    # ---------- 2. token 级 mask ----------
    conditioning_mask = torch.zeros(B, L_total, device=condition_frame_latent.device)
    conditioning_mask[:, :L_cond] = 1.0

    cond_indicator = torch.zeros(1, L_total, device=condition_frame_latent.device)
    cond_indicator[:, :L_cond] = 1.0

    # ---------- 3. token 级噪声 ----------
    noise = randn_tensor((B, L_total, C), generator=generator,
                         device=condition_frame_latent.device,
                         dtype=condition_frame_latent.dtype)

    # ---------- 4. 混合 ----------
    rand_ff = random.random() * noise_to_condition_frames
    first_mask = conditioning_mask.unsqueeze(-1) * (1.0 - rand_ff)
    latents = init_latents * first_mask + noise * (1. - first_mask)
