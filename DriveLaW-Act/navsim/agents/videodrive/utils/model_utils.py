...
def is_legacy_video_ig_state_dict_key(key: str) -> bool:
    """Drop video-branch IG tensors from DualFlow / legacy planning checkpoints."""
    if key.startswith("action_ig_heads."):
        return False
    if ".ig_heads." in key or key.startswith("ig_heads."):
        return True
    if key.endswith("ig_head.weight") or key.endswith("ig_head.bias"):
        return True
    if "ig_scale_shift_table" in key and "action_ig_heads" not in key:
        if "ig_heads" in key or key.startswith("transformer.ig_"):
            return True
    return False


def filter_legacy_video_ig_keys(state_dict: dict[str, torch.Tensor]) -> tuple[dict[str, torch.Tensor], list[str]]:
    dropped = [k for k in state_dict if is_legacy_video_ig_state_dict_key(k)]
    if not dropped:
...
def load_checkpoints(model, pretrained_ckpt, strict=False, ignore_mismatched_sizes=True):
    ...
    state_dict, dropped_ig = filter_legacy_video_ig_keys(state_dict)
    if dropped_ig:
        print(">>> Dropped legacy video IG keys from checkpoint (%d tensors)" % len(dropped_ig))
...
def load_diffusion_model(model_cls, model_dir, load_weights=True, args=None, **kwargs):
    model = model_cls(**kwargs)
    print("diffusion model", model_dir)
    if load_weights:
        from navsim.agents.videodrive.ig_setup import ensure_planning_aux_modules_before_load

        ensure_planning_aux_modules_before_load(model, args=args, checkpoint_path=model_dir)
        ...
        setup_cross_modal_from_args(model, args)
    return model
...
def forward_pass(
    ...
    **kwargs,
    ...
    if return_cross_modal_align and cross_modal_block_indices:
        model_kw["return_cross_modal_align"] = True
        model_kw["cross_modal_block_indices"] = list(cross_modal_block_indices)
        model_kw["cross_modal_align_stopgrad_video"] = bool(cross_modal_align_stopgrad_video)
    denoised_latents = model(**model_kw)[0]
