...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if len(conditions) > 0:
            ...
            # Strict Fast-WAM encoder (test-time): no stochastic "future" latents — hold-out tail by repeating
            # the last real conditioned latent frame, and mark all latent frames as fixed conditioning so the
            # video scheduler does not denoise any token (see infer loop `tokens_to_denoise_mask`).
            if fast_wam_strict and len(extra_conditioning_latents) == 0:
                any_strength = condition_latent_frames_mask > 0.5
                if any_strength.any():
                    last_f = int(any_strength.nonzero(as_tuple=False)[:, 1].max().item())
                    if last_f < num_latent_frames - 1:
                        latents[:, :, last_f + 1 :] = latents[:, :, last_f : last_f + 1]
                condition_latent_frames_mask[:, :] = 1.0
    ...
    def infer(
        ...
        **kwargs,
        ...
        # Fast-WAM strict (infer only): 4 observation latents, no extra noise/hold-out temporal slots (default 6 = 4+2).
        main_num_latent_frames = 4 if fast_wam_strict else 6
        latents, conditioning_mask, video_coords, extra_conditioning_num_latents = self.prepare_latents(
            ...
            device=device,
            dtype=torch.float32,
            fast_wam_strict=fast_wam_strict,
        )

        video_coords = video_coords.float()
        #video_coords[:, 0] = video_coords[:, 0] * (1.0 / frame_rate)

        nf_transformer = main_num_latent_frames

        if fast_wam_strict:
            fast_wam_inference = True
        ...
        def _repeat_multi_modal(x,num):
            if x is None:
                return None
            if x.ndim == 1:
                return x.unsqueeze(0).repeat(num, 1)
            if x.ndim == 2:
                return x.repeat(num, 1)
            raise ValueError(f"context_dict tensor must be 1D or 2D, got shape {tuple(x.shape)}")

        if context_dict is not None:
            context_dict = {
                k: _repeat_multi_modal(v, num_videos_per_prompt) for k, v in context_dict.items()
            }
        ...
        for i, (t, t_a) in enumerate(zip(timesteps, timesteps_a)):
                ...
                seq_len_m = conditioning_mask_model_input.shape[1]
                timestep = t.expand(latent_model_input.shape[0], seq_len_m).float()
                if is_conditioning_image_or_video:
                ...
                if compute_video:
                    ...
                    if is_conditioning_image_or_video:
                        ...
                        tokens_to_denoise_mask = (t / 1000 - 1e-6 < (1.0 - cm)).unsqueeze(-1)
        ...
        if output_type == "latent":
            if ctx_tokens is None:
                raise RuntimeError(
                    "output_type='latent' requires ctx_tokens from step 0 video forward; "
                    "ensure return_video=False still runs compute_video at i==0 with conditioning."
                )
            video = ctx_tokens
        else:
