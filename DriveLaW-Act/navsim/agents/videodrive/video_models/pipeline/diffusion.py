    ...
    def infer(
        ...
        **kwargs,
        ...
        if timesteps is None:
            # 生成连续 t \in [0,1]，再离散到视频 scheduler 的索引空间
            t_video = linear_quadratic_schedule(num_inference_steps)                  # 连续 [0,1]
            timesteps = torch.round(t_video * (NV - 1)).long()


        # ——检索真正要跑的步子（各自 scheduler）——
        ...
        for i, (t, t_a) in enumerate(zip(timesteps, timesteps_a)):
                ...
                if return_action:
                    ...
                    action_timesteps = action_t_int.expand(batch_size, action_chunk)  # (B,L)
                ...
                if return_action:
                    if self.do_classifier_free_guidance:
                        pred_v_uncond, pred_v = noise_pred["action"].float().chunk(2)
                        pred_v = pred_v_uncond + guidance_scale * (pred_v - pred_v_uncond)
                    else:
                        pred_v = noise_pred["action"].float()

                    # FM Euler 积分
                    # num_inference_steps 与 timesteps 的长度一致，dt = 1/N
