...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def infer(
        ...
        action_temperature: float = 1.10,
        action_sigma_scale: float = 1.0,  
        **kwargs,
    ):
        r"""
        ...
        if return_action:
            ...
            actions = randn_tensor((action_bs, action_chunk, action_dim), device=device, dtype=prompt_embeds.dtype, generator=action_generator) * action_temperature
        else:
            actions = None
        ...
        if action_sigma_scale != 1.0:
            # 将 action 调度器的 sigmas 放大/缩小，放大会更发散
            sigmas_a = self.scheduler_action.sigmas.clone()
            scaled = torch.clamp(sigmas_a * action_sigma_scale, min=0.0, max=1.0)
            timesteps_a, _ = retrieve_timesteps(self.scheduler_action, len(timesteps), device, (scaled * 1000).tolist())
