...
class ActionTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        q_input_for_attn2 = hidden_states
        ...
        if collect_cross_modal_align:
            from navsim.agents.videodrive.cross_modal_align import compute_cross_attn_probs_for_align

            video_hs_for_align = (
                encoder_hidden_states.detach()
                if cross_modal_align_stopgrad_video
                else encoder_hidden_states
            )
            attn_probs_align = compute_cross_attn_probs_for_align(
                self.attn2,
                q_input_for_attn2,
                video_hs_for_align,
                attention_mask=encoder_attention_mask,
            ...
            align_pack = {
                "h_act": hidden_states,
                "attn_probs_align": attn_probs_align,
                "video_hs": video_hs_for_align,
            }
            return hidden_states, align_pack
